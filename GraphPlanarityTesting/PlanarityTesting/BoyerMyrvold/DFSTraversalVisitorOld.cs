namespace GraphPlanarityTesting.PlanarityTesting.BoyerMyrvold
{
	...
	public class DFSTraversalVisitorOld<T> : BaseDFSTraversalVisitor<T>
	{
		...
		public DFSTraversalVisitorOld(Dictionary<T, int> dfsNumber, Dictionary<T, T> parent, Dictionary<T, int> lowPoint, Dictionary<T, int> leastAncestor, Dictionary<T, IEdge<T>> dfsEdge)
		{
			DFSNumber = dfsNumber;
			Parent = parent;
			LowPoint = lowPoint;
